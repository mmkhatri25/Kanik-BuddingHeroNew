        ...
        public UiState state
        {
            ...
            set
            {
                ...
                case UiState.SHOP:
                    ...
                    this.panelShop.isHubMode = false;
                    this.isScrollViewLocked = true;
                    UiManager.zeroScrollViewContentY = (!this.IsShopState(this._state) && this._state != UiState.CURRENCY_WARNING && (this._state != UiState.GENERAL_POPUP) && (this._state != UiState.SOCIAL_REWARD_POPUP || this.panelSocialRewardPopup.previousState != UiState.SHOP));
...
//GC.Collect();
            ...
            else if (!TutorialManager.IsThereTutorialCurrently() && !OldArtifactsConverter.DoesPlayerNeedsConversion(this.sim) && (TutorialManager.artifactOverhaul == TutorialManager.ArtifactOverhaul.FIN || TutorialManager.artifactOverhaul == TutorialManager.ArtifactOverhaul.BEFORE_BEGIN) && !this.IsInUnclosableMenus() && !this.loadingTransition.IsPlaying())
            {
                ...
                else
                {
                    ...
                    if (this.checkImmidiateStateTimer > 1f && this.state == UiState.NONE && uiState == UiState.NONE)
                    {
                        ...
                        /*if (!StoreManager.askedToAllowNotifications && !StoreManager.areNotificationsAllowed &&
                            this.state == UiState.NONE && uiState == UiState.NONE && TutorialManager.IsShopTabUnlocked())
                        {
                ...
                /*  PanelGeneralPopup panelGeneralPopup2 = this.panelGeneralPopup;
                    PanelGeneralPopup.State state = (this.state != UiState.HUB) ? PanelGeneralPopup.State.SERVER_REWARD :
                        PanelGeneralPopup.State.SERVER_REWARD_HUB;
                    string text3 = title;
                    string text2 = text4;
        ...
        private void UpdateTabBar(World activeWorld)
        {
            if (TutorialManager.IsHeroesTabUnlocked())
            {
                ...
                int numNotifications2 = this.sim.GetNumFreeLootpacks() + num + this.sim.numUnseenTrinketPacks;// + num2 + ((!this.sim.IsThereNotificationsPendingFromSecondAnniversaryOffer()) ? 0 : 1);
        ...
        private void UpdateTutorialStateChecks(bool isModeInAction)
        {
            ...
            if (!TutorialManager.IsThereTutorialCurrently())
            {
                ...
                else if (TutorialManager.christmasTreeEventUnlocked == TutorialManager.ChristmasTreeEventUnlocked.BEFORE_BEGIN &&
                    (this.state == UiState.NONE || this.state == UiState.HUB) &&
                    this.sim.IsChristmasTreeEnabled())// && this.openOfferPopupButton.gameObject.activeSelf)
                {
        ...
        private void UpdateShopScreen()
        {
            ...
            bool isLookingAtOffers = false;// panelShop.isLookingAtOffers;
            ...
            if (!isLookingAtOffers)
            {
                ...
                while (i < num2)
                {
                    ...
                    if (i == 0)
                    {
                        //panelShop.shopLootPacks[i].gameObject.SetActive(false);
                        int numFreeLootpacks = 1;// this.sim.GetNumFreeLootpacks();
                        ...
                        print("Difference: " + difference.Hours + " Minutes :"+ difference.Minutes);
                        ...
                        //merchantItem.textPrice.text = "FREE";// text + ((text.Length <= 0 || text2.Length <= 0) ? string.Empty : "\n") + text2;
                    ...
                    merchantItem.buttonUpgradeAnim.GetComponent<GameButton>().interactable  = this.sim.CanOpenLootpack(shopPack);
        ...
        /*DateTime datevalue3 = System.DateTime.Now;
            double hours = (datevalue3 - datevalue1).TotalHours;
            Debug.Log("It has been " + hours.ToString() + " hours since the beginning of the year");
            MerchantItem merchantItem = panelShop.shopLootPacks[0];
            if (hours > 4)
            {
                merchantItem.gameButton.interactable = true;
                merchantItem.textPrice.text = "FREE";
            }
            else
            {
                merchantItem.gameButton.interactable = false;
                merchantItem.textPrice.text = "Try again after "+(4-(int)hours)+ " hours";
                merchantItem.textPrice.color = Color.white;
            }*/
        ...
        private void SetShopLootpackSummaryScreen()
        {
            ...
            int k = 0;
            int count3 = this.sim.lootDropGears.Count-1;
            while (k < count3)
            {
        ...
        System.DateTime datevalue1;
        private void OnClickedShopLootpackSelect()
        {
             datevalue1 = System.DateTime.Now;
    if (this.shopLootpackSelect.timer < this.shopLootpackSelect.targetTimer)
        ...
        private void OnClickedAcceptServerReward(RewardOrigin rewardOrigin)
        {
            ...
            if (!this.acceptingServerReward)
            {
                this.command = new UiCommandServerRewardCollect
                {
                    ...
                    isHub = false,// (this.panelGeneralPopup.state == PanelGeneralPopup.State.SERVER_REWARD_HUB),
        ...
        {
            return state == UiState.HUB || state == UiState.SUPPORT_POPUP || state == UiState.HUB_ACHIEVEMENTS ||
                state == UiState.HUB_MODE_SETUP || state == UiState.HUB_OPTIONS || state == UiState.HUB_MODE_SETUP_HERO ||
                state == UiState.HUB_MODE_SETUP_TOTEM || state == UiState.HUB_DATABASE_HEROES_ITEMS || state == UiState.HUB_DATABASE_TOTEMS || 
                state == UiState.HUB_DATABASE_TRINKETS || state == UiState.HUB_DATABASE_EVOLVE || state == UiState.HUB_CREDITS || 
                state == UiState.UPDATE_REQUIRED || (state == UiState.CURRENCY_WARNING && this.currencyWarningInHub) || 
                (state == UiState.GENERAL_POPUP &&((this.IsInHubMenus(this.panelChristmasOffer.previousState, state))) || (state == UiState.CHANGE_SKIN && 
                this.panelHeroSkinChanger.oldState == UiState.HUB_DATABASE_HEROES_ITEMS) || (state == UiState.SKIN_BUYING_WARNING && 
                this.panelHeroSkinChanger.oldState == UiState.HUB_DATABASE_HEROES_ITEMS) || state == UiState.HUB_CHARMS || 
                state == UiState.CHARM_INFO_POPUP || state == UiState.HUB_ARTIFACTS || (state == UiState.ARTIFACTS_INFO && 
                this.panelArtifactsInfo.stateToReturn == UiState.HUB_ARTIFACTS) || state == UiState.RIFT_SELECT_POPUP || 
                state == UiState.HUB_SHOP || state == UiState.HUB_DATABASE_HEROES_SKILLS || (state == UiState.CURRENCY_WARNING && 
                this.currencyWarningInHub) || (state == UiState.RIFT_EFFECTS_INFO && (this.panelRiftEffectInfo.stateToReturn == UiState.HUB_MODE_SETUP || 
                this.panelRiftEffectInfo.stateToReturn == UiState.RIFT_SELECT_POPUP)) || (state == UiState.SOCIAL_REWARD_POPUP && 
                this.IsInHubMenus(this.panelSocialRewardPopup.previousState, state)) || state == UiState.HUB_OPTIONS_WIKI || 
                (state == UiState.TRINKET_INFO_POPUP && this.IsInHubMenus(this.panelTrinketInfoPopup.stateToReturn, state)) ||
                (state == UiState.TRINKET_EFFECT_SELECT_POPUP && !this.panelTrinketSmithing.isBattlefield) || (state == UiState.TRINKET_RECYCLE_POPUP && 
                this.IsInHubMenus(this.panelTrinketRecycle.previousState, state)) || (state == UiState.CHRISTMAS_PANEL &&
                this.IsInHubMenus(this.panelChristmasOffer.previousState, state)) || (state == UiState.CHRISTMAS_OFFERS_INFO_POPUP && 
                this.IsInHubMenus(this.panelChristmasOffer.previousState, state)) || (state == UiState.CHRISTMAS_CANDY_TREAT_POPUP &&
                this.IsInHubMenus(this.panelChristmasOffer.previousState, state)) || (state == UiState.SHOP_LOOTPACK_SELECT &&
                (this.shopLootpackSelect.previousState == UiState.HUB_SHOP || (this.shopLootpackSelect.previousState == UiState.SECOND_ANNIVERSARY_POPUP && 
                this.secondAnniversaryPopup.previousState == UiState.HUB_SHOP))) || ((state == UiState.SHOP_LOOTPACK_OPENING || state == UiState.SHOP_LOOTPACK_SUMMARY ||
                state == UiState.SHOP_MINE || state == UiState.BUY_FLASH_OFFER_CHARM || state == UiState.SHOP_CHARM_PACK_SELECT || state == UiState.SHOP_CHARM_PACK_OPENING ||
                state == UiState.SELECT_TRINKET || state == UiState.SHOP_TRINKET_OPEN_POPUP || state == UiState.BUY_FLASH_OFFER) && 
                this.panelShop.isHubMode) || state == UiState.PATCH_NOTES || (state == UiState.ARTIFACTS_CRAFT && 
                this.panelArtifactsCraft.uistateToReturn == UiState.HUB_ARTIFACTS) ||
                ((state == UiState.ARTIFACT_SELECTED_POPUP || state == UiState.ARTIFACT_EVOLVE || state == UiState.POSSIBLE_ARTIFACT_EFFECTS_POPUP) 
                && this.IsInHubMenus(this.panelArtifactPopup.stateToReturn, state)) || state == UiState.ARTIFACT_OVERHAUL || 
                (state == UiState.SECOND_ANNIVERSARY_POPUP && this.secondAnniversaryPopup.previousState == UiState.HUB_SHOP) || 
                (state == UiState.NEW_TAL_MILESTONE_REACHED_POPUP && this.IsInHubMenus(this.newTALMilestoneReachedPopup.previousState)) || 
                (state == UiState.CAN_NOT_EVOLVE_ARTIFACT_POPUP && (this.notEvolveArtifactPopup.previousState == UiState.HUB_ARTIFACTS || 
                (this.notEvolveArtifactPopup.previousState == UiState.ARTIFACT_SELECTED_POPUP && this.panelArtifactPopup.stateToReturn == UiState.HUB_ARTIFACTS))) || 
                (state == UiState.NEW_TAL_MILESTONE_REACHED_POPUP && this.panelArtifactPopup.stateToReturn == UiState.HUB_ARTIFACTS) ||
                (state == UiState.POSSIBLE_ARTIFACT_EFFECTS_POPUP && this.posibleArtifactEffectPopup.stateToReturn == UiState.HUB_ARTIFACTS) ||
                (state == UiState.ARTIFACT_SELECTED_POPUP && this.panelArtifactPopup.stateToReturn == UiState.HUB_ARTIFACTS));
        }
